...
function createNebula() {
	...
	for (var i = 0; i < nebulaColors.length; i++) {
		var cloud = new THREE.Mesh(
			new THREE.SphereGeometry(500 + i * 120, 6, 5),
	...
	for (var p = 0; p < planetData.length; p++) {
		...
		var planet = new THREE.Mesh(
			new THREE.IcosahedronGeometry(pd.radius, 1),
...
var RocketShip = function () {
	...
	this.finTop = new THREE.Mesh(geomFin, this.matHull);
	this.finBottom = new THREE.Mesh(geomFin, this.matHull);
	this.finTop.position.set(0, 12, -12);
	this.finBottom.position.set(0, -12, -12);
	this.finTop.castShadow = true;
	this.finBottom.castShadow = true;
	this.mesh.add(this.finTop);
	this.mesh.add(this.finBottom);

	// Cockpit canopy
	var geomCanopy = new THREE.SphereGeometry(15, 10, 8, 0, Math.PI * 2, 0, Math.PI / 1.8);
	this.canopy = new THREE.Mesh(geomCanopy, new THREE.MeshPhongMaterial({ color: Colors.glass, transparent: true, opacity: .55, shading: THREE.FlatShading }));
	this.canopy.position.set(0, 12, 28);
...
// decorative stripe (low-poly decal)
RocketShip.prototype.setStripe = function (enabled, hex) {
	if (enabled) {
		if (!this.stripe) {
			var geom = new THREE.BoxGeometry(44, 4, 2);
			this.stripe = new THREE.Mesh(geom, new THREE.MeshPhongMaterial({ color: hex || 0xffffff, shading: THREE.FlatShading }));
			this.stripe.position.set(0, 6, 36);
			this.mesh.add(this.stripe);
		}
		if (this.stripe.material) this.stripe.material.color.setHex(hex || 0xffffff);
...
RocketShip.prototype.setFinScale = function (s) {
	if (this.finTop && this.finBottom) {
		this.finTop.scale.set(s, s, s);
		this.finBottom.scale.set(s, s, s);
	}
};

RocketShip.prototype.setSize = function (s) {
	if (this.mesh) this.mesh.scale.set(s, s, s);
};

RocketShip.prototype.setCanopyColor = function (hex) {
	if (this.canopy && this.canopy.material) this.canopy.material.color.setHex(hex);
};

RocketShip.prototype.addScratch = function () {
	if (!this.mesh) return;
	var mat = new THREE.MeshBasicMaterial({ color: 0x131416, transparent: true, opacity: 0.95 });
	var geom = new THREE.PlaneGeometry(18, 4);
	var scratch = new THREE.Mesh(geom, mat);
	var rx = (Math.random() * 2 - 1) * 10;
	var ry = (Math.random() * 2 - 1) * 8 + 6;
	var rz = Math.random() * 20 + 10;
	scratch.position.set(rx, ry, rz);
	scratch.rotation.set((Math.random() - .5) * 0.6, (Math.random() - .5) * 0.6, (Math.random() - .5) * 0.6);
	scratch.userData = { life: 0, maxLife: 9 };
	this.mesh.add(scratch);
	if (!this.scratches) this.scratches = [];
...
function createShip() {
	...
	// gameplay properties
	ship.hullIntegrity = 5; // number of scratches until failure (easier start)
	ship.scratches = [];
	ship.velocityX = 0;
	ship.velocityY = 0;
...
function currentAsteroidSpeed() {
	// gentler initial speed and slower ramp
	return 3.2 + Math.min(6.8, elapsedTime * 0.02);
}
...
function createAuroraMap() {
	...
	for (var j = 0; j < 4; j++) {
		...
		auroraObstacles.push({ mesh: ring, type: 'ring', radius: 42 + j * 8, phase: j * 0.7, yBase: 90 + j * 20, xBase: (j - 1.5) * 120, zBase: -1200 - j * 360, hitCooldown: 0 });
		auroraObstacles.push({ mesh: crystal, type: 'crystal', radius: 18, phase: j * 0.4, yBase: 90 + j * 20, xBase: (j - 1.5) * 120, zBase: -1180 - j * 360, hitCooldown: 0 });
	}
...
function spawnEnemy(e, initialSpread) {
	...
	e.speed = ENEMY_SPEED_MIN + Math.random() * 1.2 + level * 0.15;
	e.fireCooldown = 2.2 + Math.random() * 2.5; // slower firing initially
...
function createEnemyField() {
	var count = 0; // start with no immediate enemy ships to keep first stretch easy
...
function updateEnemies(dt) {
	for (var i = 0; i < enemies.length; i++) {
		...
		if (gameState === 'playing') {
			...
			if (e.fireCooldown <= 0 && Math.abs(dxToShip) < 70 && dzToShip < -20 && dzToShip > SPAWN_Z * 0.5) {
				fireEnemyLaser(e);
				e.fireCooldown = 2.6 + Math.random() * 1.8 - level * 0.03;
			}
...
function updateEnemyLasers(dt) {
	for (var i = enemyLasers.length - 1; i >= 0; i--) {
		...
		if (gameState === 'playing' && !ship.invulnerable) {
...
function updateScratches(dt) {
	if (!ship || !ship.scratches) return;
	for (var i = ship.scratches.length - 1; i >= 0; i--) {
		var s = ship.scratches[i];
		s.userData.life += dt;
		var t = s.userData.life / s.userData.maxLife;
		s.material.opacity = Math.max(0.12, 0.95 * (1 - t));
		if (s.userData.life > s.userData.maxLife) {
			// slowly remove old scratches
			try { ship.mesh.remove(s); } catch (e) {}
			ship.scratches.splice(i, 1);
		}
	}
}
...
function updateHud() {
	...
	if (hud && !document.getElementById('hull-line')) {
		var hullLine = document.createElement('div');
		hullLine.id = 'hull-line';
		hullLine.textContent = 'Hull: ' + (ship && ship.hullIntegrity !== undefined ? ship.hullIntegrity : 0);
		hud.appendChild(hullLine);
	} else if (document.getElementById('hull-line')) {
		document.getElementById('hull-line').textContent = 'Hull: ' + (ship && ship.hullIntegrity !== undefined ? ship.hullIntegrity : 0);
	}
}
...
function triggerGameOver() {
	...
	if (lives > 0) {
		var restoreShip = function () {
			...
			if (ship.scratches) {
				for (var si = ship.scratches.length - 1; si >= 0; si--) {
					try { ship.mesh.remove(ship.scratches[si]); } catch (e) {}
				}
				ship.scratches = [];
			}
			showAlert('Auto-repair complete! Continue flying.');
			updateHud();
		};
		setTimeout(restoreShip, 800);
		return;
	}

	// Out of lives — the run is actually over now.
	gameState = 'gameover';
...
function restartGame() {
	...
	nextWaveAt = 6; // give the player a longer calm start
	ship.mesh.visible = cameraMode !== 'cockpit';
	ship.mesh.position.set(0, 130, 0);
	ship.mesh.rotation.set(0, Math.PI, 0);

	// reset hull and scratches
	if (ship) {
		ship.hullIntegrity = 5;
		ship.invulnerable = false;
		if (ship.scratches) {
...
function updateShip() {
	...
	// apply a more physical turning model: velocity moves toward target, ship follows velocity
	var desiredXVel = (targetX - ship.mesh.position.x) * 0.18;
	var desiredYVel = (targetY - ship.mesh.position.y) * 0.18;
	ship.velocityX += (desiredXVel - ship.velocityX) * 0.12;
	ship.velocityY += (desiredYVel - ship.velocityY) * 0.12;

	ship.mesh.position.x += ship.velocityX;
	ship.mesh.position.y += ship.velocityY;
	ship.mesh.position.z -= 0.7 * forwardBoost;

	// rotation follows the velocity for smoother, more logical banking
	ship.mesh.rotation.z = ship.velocityX * -0.045;
	ship.mesh.rotation.x = (ship.velocityY) * -0.0075 + (ship.mesh.position.y - targetY) * 0.0015;
	ship.mesh.rotation.y = Math.PI + ship.velocityX * 0.0045;

	// flame pulse and boost visuals
	var basePulse = 1 + Math.sin(elapsedTime * 22) * 0.12;
	if (boostActive) {
		var boostPulse = 1.65 + Math.sin(elapsedTime * 48) * 0.28;
		ship.flameCore.scale.set(boostPulse, 1.2, boostPulse);
		ship.flameOuter.scale.set(boostPulse * 1.15, 1.05, boostPulse * 1.15);
		if (ship.engineGlow) ship.engineGlow.intensity = 2.6;
		try { ship.flameCore.material.opacity = 1.0; ship.flameOuter.material.opacity = 0.85; } catch (e) {}
	} else {
		ship.flameCore.scale.set(basePulse, 1, basePulse);
		ship.flameOuter.scale.set(basePulse * .9, 1, basePulse * .9);
		if (ship.engineGlow) ship.engineGlow.intensity = 1.4;
		try { ship.flameCore.material.opacity = 0.9; ship.flameOuter.material.opacity = 0.5; } catch (e) {}
	}
}
...
function updateAsteroids() {
	...
	if (waveTimer > nextWaveAt) {
		waveTimer = 0;
		// make waves come less frequently early on
		nextWaveAt = Math.max(4.5, 7.5 - level * 0.08);
		showAlert('Wave incoming!');
		for (var s = 0; s < Math.min(2 + Math.floor(level / 3), 4); s++) {
			var extra = {};
			spawnAsteroid(extra, true);
			asteroids.push(extra);
		}
		if (level >= 4 && enemies.length < Math.min(1 + Math.floor(level / 3), 3)) {
			var extraEnemy = {};
	...
	for (var i = 0; i < asteroids.length; i++) {
		...
		if (gameState === 'playing' && !ship.invulnerable) {
			...
			if (dist < a.radius * a.mesh.scale.x + 28) {
				// light collision with asteroid — apply scratch and minor push
				if (ship.hullIntegrity > 0) {
					ship.hullIntegrity -= 1;
					ship.addScratch();
					spawnExplosion(ship.mesh.position, 0x7cf7ff, 0.6);
					triggerShake(2, 0.18);
					showAlert('Hull scratched! Integrity: ' + ship.hullIntegrity);
					// respawn the asteroid to avoid immediate re-collision
					spawnAsteroid(a, false);
					if (ship.hullIntegrity <= 0) {
						triggerGameOver();
					}
...
function updateAuroraHazards(dt) {
	...
	for (var i = 0; i < auroraObstacles.length; i++) {
		...
		if (obstacle.hitCooldown > 0) obstacle.hitCooldown -= dt;

		if (gameState === 'playing' && obstacle.hitCooldown <= 0) {
			...
			if (dist < obstacle.radius + 16) {
				...
				score += 20;
				obstacle.hitCooldown = 1.4;
				showAlert(obstacle.type === 'crystal' ? 'Crystal bonus! +20' : 'Aurora ring bonus! +20');
			}
		}
	}
}
...
function init() {
	...
	// plane designer controls
	var canopyInput = document.getElementById('canopy-color');
	if (canopyInput) {
		canopyInput.addEventListener('input', function () {
			var hex = parseInt(this.value.replace('#', ''), 16);
			ship.setCanopyColor(hex);
		});
	}
	var hullSize = document.getElementById('hull-size');
	if (hullSize) {
		hullSize.addEventListener('input', function () {
			var val = parseFloat(this.value);
			ship.setSize(val);
		});
	}
	var finScale = document.getElementById('fin-scale');
	if (finScale) {
		finScale.addEventListener('input', function () {
			var val = parseFloat(this.value);
			ship.setFinScale(val);
		});
	}

	var stripeEnable = document.getElementById('stripe-enable');
	var stripeColor = document.getElementById('stripe-color');
	if (stripeEnable) {
		stripeEnable.addEventListener('change', function () {
			var enabled = !!this.checked;
			var hex = parseInt((stripeColor && stripeColor.value ? stripeColor.value.replace('#', '') : 'ffffff'), 16);
			ship.setStripe(enabled, hex);
		});
	}
	if (stripeColor) {
		stripeColor.addEventListener('input', function () {
			var hex = parseInt(this.value.replace('#', ''), 16);
			if (stripeEnable && stripeEnable.checked) ship.setStripe(true, hex);
		});
	}

	// jukebox: play/pause and volume control (expects space.mp3 at project root)
	var jukebox = document.getElementById('jukebox');
	var jukeboxBtn = document.getElementById('jukebox-toggle');
	var jukeboxVol = document.getElementById('jukebox-volume');
	if (jukebox) {
		jukebox.volume = 0.6;
		if (jukeboxBtn) {
			jukeboxBtn.addEventListener('click', function (e) {
				e && e.preventDefault();
				if (jukebox.paused) {
					var p = jukebox.play();
					if (p && p.then) {
						p.then(function () {
							jukeboxBtn.textContent = 'Pause';
							showAlert('Music playing');
							// set playing visuals
							document.getElementById('jukebox-radio').classList.add('playing');
						}).catch(function () {
							showAlert('Click Play to enable audio');
							jukeboxBtn.textContent = 'Play';
						});
					} else {
						jukeboxBtn.textContent = 'Pause';
						// set playing visuals
					...
					jukebox.pause();
					jukeboxBtn.textContent = 'Play';
					// remove playing visuals
					document.getElementById('jukebox-radio').classList.remove('playing');
				}
			});
		}
		if (jukeboxVol) {
			jukeboxVol.addEventListener('input', function () { jukebox.volume = parseFloat(this.value); });
			jukeboxVol.value = jukebox.volume;
		}
	}
	...
	// show designer overlay at start; loop runs but gameState is 'design' so hazards won't move
	var startBtn = document.getElementById('start-flight');
	var overlay = document.getElementById('designer-overlay');
	var rndBtn = document.getElementById('randomize-design');
	var hudEl = document.getElementById('hud');
	// hide HUD while designing
	if (hudEl && gameState === 'design') hudEl.classList.add('hidden');
	if (rndBtn) {
		rndBtn.addEventListener('click', function () {
			// randomize color, canopy, size, fin, stripe
			var colors = shipColors;
			var c = colors[Math.floor(Math.random() * colors.length)].hex;
			ship.recolor(c);
			var canopyHex = Math.floor(Math.random() * 0xffffff);
			ship.setCanopyColor(canopyHex);
			var s = 0.5 + Math.random() * 0.9;
			ship.setSize(s);
			var f = 0.6 + Math.random() * 1.0;
			ship.setFinScale(f);
			var stripeOn = Math.random() > 0.5;
			var stripeHex = Math.floor(Math.random() * 0xffffff);
			ship.setStripe(stripeOn, stripeHex);
			// update HUD inputs if present
			var hullSize = document.getElementById('hull-size'); if (hullSize) hullSize.value = ship.mesh.scale.x.toFixed(2);
			var finScale = document.getElementById('fin-scale'); if (finScale) finScale.value = f.toFixed(2);
			var canopy = document.getElementById('canopy-color'); if (canopy) canopy.value = '#'+('000000'+canopyHex.toString(16)).slice(-6);
			var stripeEnable = document.getElementById('stripe-enable'); if (stripeEnable) stripeEnable.checked = !!stripeOn;
			var stripeColor = document.getElementById('stripe-color'); if (stripeColor) stripeColor.value = '#'+('000000'+stripeHex.toString(16)).slice(-6);
			showAlert('Design randomized');
		});
	}
	if (startBtn) {
